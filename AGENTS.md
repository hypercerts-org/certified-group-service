...
- **Synchronous SQLite is load-bearing for concurrency**: the driver is `better-sqlite3` (sync) on Node's single event loop, so a `raw.transaction(...)` runs to completion without yielding — handlers cannot interleave _within_ a transaction, only at `await` boundaries _between_ DB calls. Several read-modify-write flows depend on this. Indivisible statements are not sufficient on their own, though: a validate-then-write sequence spanning an `await` can still act on state another handler invalidated in between, so the ownership operations (`ownershipTransfer.propose`/`accept`/`cancel` and `admin.setOwner`) run under a per-group serialization boundary — `ctx.ownershipLock.run(groupDid, fn)` (`src/transfer/ownership-lock.ts`), an in-process promise queue per group. Hold it around validation plus mutation only; resolution (network I/O) stays outside. `member.remove`/`role.set` stay outside it, so `accept` re-reads its recipient's membership immediately before `transferOwner`; and because the lock dies with the process, two writes that must not come apart across a crash are ordered so the harmless half survives (`admin.setOwner` clears the pending proposal _before_ it transfers). Don't switch to an async driver (`node:sqlite` worker, libsql, parallel pool), or run several processes against one group DB, without re-auditing these flows. See `src/db/sqlite.ts` and the Concurrency model in `docs/architecture.md`.
